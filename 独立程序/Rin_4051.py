...
DEFAULT_POINTS = 1001
DEFAULT_SEGMENTS = [
    (10, 100, 50, 1, "File.DAT"),
    (100, 1000, 5, 10, "File_001.DAT"),
    (1000, 10000, 30, 10, "File_002.DAT"),
...
class Rin_4051:
    ...
    def single_sweep_fetch(self, prefer_binary=True):
        ...
        # 发送单次 *OPC? 让仪器自行阻塞等待，直到扫描完成
        try:
            #self.log(f"[等待扫描] 已发送 *OPC?，最大等待超时时间已被设为当前波段的动态超时...")
            # PyVISA 底层会根据 self.inst.timeout 自动阻塞，不需要自己写 while 循环
            opc_result = self.query("*OPC?")
            if opc_result.strip() != "1":
                self.log("警告: *OPC? 未返回 1")
        except Exception as e:
            self.log(f"[警告] 等待 *OPC? 时发生超时或异常: {e}")
        # start = time.time()
    ...
    def _process_data(self):
        ...
        for i, L in enumerate(seg_lengths):
            ...
            if i < len(self.segments):
                rbw = self.segments[i][2]
                scale[idx0:idx1] = math.sqrt(rbw)
            else:
...
class Rin_4051_GUI:
    ...
    def visualize_data(self):
        ...
        try:
            ...
            self.log(f"[可视化] 图像已自动保存至: {png_path}")
        except Exception as e:
            self.log(f"[错误] 图像保存失败: {e}")
            png_path = None
        ...
        try:
            ...
            def save_img():
                ...
                if tgt:
                    try:
                        # 保存原图（高质量），而不是缩放后的预览图
                        Image.open(img_path).save(tgt)
                        messagebox.showinfo("保存", f"已另存为: {tgt}", parent=win)
                    except Exception as e:
            ...
            if save_button_top_center:
                tk.Button(btn_frame, text="保存", command=save_img).pack(side=tk.TOP, pady=4)
            else:
                tk.Button(btn_frame, text="保存", command=save_img).pack(side=tk.LEFT, padx=6)
...
# pyinstaller --onefile --noconsole --icon="D:\Coding\Project\PreciTestSystem\PTS\PreciLasers.ico" --hidden-import=pyvisa --hidden-import=tkinter --hidden-import=PIL --hidden-import=matplotlib "D:\Coding\Project\PreciTestSystem\PTS\独立程序\Rin_4051.py"